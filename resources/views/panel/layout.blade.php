...
<head>
    ...
    <title>@yield('title', __('talkto::panel.title'))</title>
...
<body class="min-h-screen bg-slate-50 text-slate-900">
    <div class="border-b border-slate-200 bg-white">
        <div class="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
            <div>
                <p class="text-sm font-medium uppercase tracking-wide text-slate-500">{{ __('talkto::panel.brand') }}</p>
                <h1 class="text-2xl font-semibold text-slate-950">{{ __('talkto::panel.title') }}</h1>
            </div>
            <nav class="flex flex-wrap gap-2 text-sm font-medium">
                <a href="{{ route($routePrefix.'index') }}" class="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100">{{ __('talkto::panel.navigation.dashboard') }}</a>
                <a href="{{ route($routePrefix.'messages.index') }}" class="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100">{{ __('talkto::panel.navigation.messages') }}</a>
                <a href="{{ route($routePrefix.'connections.index') }}" class="rounded-md px-3 py-2 text-slate-700 hover:bg-slate-100">{{ __('talkto::panel.navigation.connections') }}</a>
            </nav>
        </div>
    </div>
